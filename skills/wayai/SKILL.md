...
| Voice calls — Realtime connection, `pilot_voice` agent, `call_utterance` steering monitor | CLI (`wayai push`) or UI — see [`references/calls.md`](references/calls.md) |
...
| `pilot_voice` | Pilot (voice of live calls) | Multiple (one answers calls) | The voice of the hub's live AI voice calls: talks with the caller and hands every question to the pilot, which answers. Binds only a Realtime connection; no text turn, eval run or transfer ever runs it — see [Voice Calls](#voice-calls) |
...
| **Realtime** | OpenAI GPT-Live — the voice of live AI voice calls, used only by a `pilot_voice` agent |
...
|---|---|
| `instruction` | Only the agent's own request. Nothing from the conversation. |
| `summary` (default) | The request + the rolling conversation summary. |
...
| 7 (OAuth connection) | `https://app.wayai.pro/settings/organizations/<org_id>/hubs/<hub_id>/connections?connector=<whatsapp\|instagram\|mcp-server>` |
