...
| `monitor` | Background | 1 per firing trigger | Observes silently, and on `user_message` its rules may act — including handing the conversation to another agent or to a human team, which takes effect on that same message. At most one enabled monitor on each of `idle`, `user_message` and `assistant_reply`; any number on `manual` |
