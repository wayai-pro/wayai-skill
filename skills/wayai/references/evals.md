...
- **No symlinks.** `attachments/` and every path component leading to it must be a real directory. If `attachments/`, the hub folder, or any folder above them is a symlink — even one pointing back inside the hub — `wayai pull` stops with an error and writes or prunes nothing, so a pre-planted link can't redirect writes. The same holds for scenarios: a symlinked scenario `.yaml`, eval set folder, scenario inside a set, or journey `.yaml` stops `wayai push` (and `diff`/`publish`) with an error, and so does `wayai pull` into an existing preview hub folder, which reads it first to show the diff. Other symlinked files there (a README) are ignored, as before.
...
Pacing is a per-run choice, not stored in the scenario YAML — set it each `run-eval` (the UI exposes the same presets).
