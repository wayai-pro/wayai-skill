...
The exceptions, all exit 1: `--no-queue` fails fast on the conflict; `--no-wait` implies it (that flag exists to free the terminal, so it must not start blocking); and a conflict that waiting cannot resolve — a rotated seed credential, a full lease ledger — is never queued. **`--timeout` is the whole command's budget**, so time spent waiting is time not spent watching the run: if it expires with the session still going, `run-eval` exits 1 (a run that scored nothing is not a success) and tells you how much of the budget the wait consumed. Raise `--timeout` on hubs where queueing is routine.
