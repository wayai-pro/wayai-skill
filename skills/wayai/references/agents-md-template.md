...
- **Per-hub `wayai-ws/hubs/<hub>/AGENTS.md`** — seeded as a scaffold on
  `wayai pull` / `wayai push`, and **absence-guarded**: a file you already have is never overwritten. See
...
files. When you work in `wayai-ws/bases/<base>/`, create `AGENTS.md` yourself
if it is missing, in the same shape as the per-hub one.
