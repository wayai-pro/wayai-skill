...
**Internal consult attachments are not addressable, on any turn.** A file attached inside a consult thread — a team member's private question to a consultant, an advisor's answer — is hidden from every agent-facing read, in this conversation and in an earlier one: it is never announced, never resolves by path, bare name or file ID, and never appears in a candidate list. The team still sees the file in the chat thread and the archive keeps every row.

That holds for the consultant's own turn too, which is deliberate: a consultant's answer is returned verbatim to whoever opened the consult, and an *agent* can open one, so a file read on a consult turn could come back to the customer as text. **This does cost one flow.** Attaching a file to an `@consultant` question and naming it in the text used to let the consultant open it by name — a consultant's context is text-only and never carries a file's path (see `roles-and-settings.md`), so the name a human typed was the only route, and it is now closed: the consultant gets "not found" and should be instructed to ask the team to paste the relevant content. Announcing consult-thread attachments in the consultant's own context, so that flow works properly, is a pending follow-up.

A hidden file still keeps its alias **reserved**: a conversation holding a consult `receipt.pdf` and a visible one announces the visible one as `conversation/receipt-2.pdf`, and `conversation/receipt.pdf` resolves for nobody. That is what keeps an address the transcript already announced pointing at the same file forever — so on the `conversation/` and `conversations/` mounts an ordinal is *not* the promotable rank it is for resources.
...
| `resource_id` | string | No | Restricts a path to one resource, named by its id or its slug (`product-docs`, or `resources/product-docs`) — use when the same file name exists in several and the reference comes back ambiguous |

\* At least one of `path` / `file_id` is required; `path` wins when both are given. `resource_id` narrows only a path, and it is checked after a resource file's `file_id` but before a conversation attachment's: a resource file the agent can read, addressed by `file_id`, is never refused because of `resource_id`, while an attachment addressed by `file_id` is refused when `resource_id` names no resource the agent is linked to — omit `resource_id` for an attachment. An unknown reference reports "File not found" with the discovery hint. "Access denied" means one of two things: a `file_id` whose file sits in a resource this agent is not linked to, or a `resource_id` naming no resource the agent is linked to — the latter reads identically whether or not that resource exists, by id or by slug, so it never confirms one does.
