...
| `wayai analytics query` | Structured query over raw column paths: `--metric "col=…,agg=…"`, `--group-by`, `--order-by`, correlations. `--metric`, `--group-by`, `--order-by` and `--filter` take `data.*` paths bare — this surface casts those for you, unlike `analytics sql`. On `--filter` that covers the ordering comparisons `gt`/`gte`/`lt`/`lte`, which read the path as a number — so a value stored as the JSON string `"7"` compares equal to `7`. The other operators are **not** cast, and on a `data.*` path that shows: `eq`/`neq` need the value to convert to every type the path is stored as across the scanned rows (one holding both `7` and `"abc"` errors on `eq abc`); `like` needs every scanned row to hold a string; `in`/`not_in` always error. Route what those cannot express through `analytics sql`, casting the path yourself with `toString(…)`. A grouped column comes back under the underscored form of its path (`data_meta_kanban_status`) |
